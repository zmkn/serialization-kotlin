...
class Jackson(
    ...
    val jsonMapper: JsonMapper = mapper.rebuild().apply {
        initializer()
    }
        .addModule(defaultKotlinModuleBuilder.apply(kotlinModuleInitializer).build())
        .build()
    ...
    companion object {
        ...
        val jsonMapper: JsonMapper = JsonMapper.builder().apply {
            // 注册 Kotlin 模块以支持 Kotlin 数据类
            changeDefaultPropertyInclusion { incl -> incl.withValueInclusion(JsonInclude.Include.NON_NULL) } // 忽略所有值为 null 的属性
            // 配置序列化特性
            enable(SerializationFeature.INDENT_OUTPUT) // 在输出时美化打印JSON格式
            enable(SerializationFeature.FAIL_ON_EMPTY_BEANS) // 使没有可序列化的属性时抛出异常
            ...
            disable(DateTimeFeature.WRITE_DATES_AS_TIMESTAMPS) // 使日期类型不会被序列化为时间戳
            // 配置反序列化特性
            ...
            configure(JsonWriteFeature.COMBINE_UNICODE_SURROGATES_IN_UTF8, true) // 在 UTF-8 编码输出中，将 Unicode 代理对（surrogate pairs）组合成单个字符
        }.also {
            it.addModule(defaultKotlinModuleBuilder.build()).addModule(DatetimeJacksonModule.all).addModule(TimeJacksonModule.all)
        }.build()
    }
}
